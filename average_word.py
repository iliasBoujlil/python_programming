...
lengthofwords = []
import random
wordamount = random.randint(3,6)
print ("I'll need", wordamount, "words:")
for i in range(1, wordamount + 1):
    word = input("Word #%s please \n>" % (i))
    listofwords.append(word)
for word in listofwords:
    length = len(word)
    lengthofwords.append(length)
average = sum(lengthofwords) / wordamount
    
sortedlist = sorted(listofwords, key = len)
print ("Shortest: %s" % (sortedlist[0]))
print ("Longest : %s" % (sortedlist[-1]))
print ("Average Length:", format(average,'.2f'))
